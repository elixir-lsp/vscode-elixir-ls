...
let workspaceSubscription: vscode.Disposable | null | undefined =
...
function configureRestart(context: ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      await Promise.all(
        allClients().map(async (client: LanguageClient) => {
          const command =
            client.initializeResult!.capabilities.executeCommandProvider!.commands.find(
...
function configureManipulatePipes(
...
export function activate(context: ExtensionContext): void {
  ...
  function didOpenTextDocument(document: vscode.TextDocument) {
    ...
    if (
      ["elixir", "eex", "html-eex", "phoenix-heex", "surface"].indexOf(
        document.languageId
      ) < 0
    ) {
      return;
    ...
    if (!folder) {
      if (
        ...
        folder = workspace.getWorkspaceFolder(
